  ...
  return (
    <div className="snapshot">
      <div className="snap-head">
        <span className="snap-title">Supply snapshot</span>
        <span className="snap-n">{priced.length} products scanned</span>
      </div>

      <div className="stats">
        <div className="stat"><div className="s-label">Price floor</div><div className="s-value dim-cur"><em>{currency} </em>{fmt(lo)}</div></div>
        <div className="stat"><div className="s-label">Median</div><div className="s-value dim-cur"><em>{currency} </em>{fmt(med)}</div></div>
        <div className="stat"><div className="s-label">Ceiling</div><div className="s-value dim-cur"><em>{currency} </em>{fmt(hi)}</div></div>
        <div className="stat"><div className="s-label">Avg rating</div><div className="s-value">★ {avgRating}</div></div>
      </div>

      <div className="ladder">
        <div className="s-label" style={{ marginBottom: 8 }}>Price ladder · coloured by rating</div>
        {priced
          .slice()
          .sort((a, b) => a.price - b.price)
          .map((p, i) => (
            <div className="rung" key={p.asin || i} style={{ animationDelay: `${i * 50}ms` }}>
              <span className="rung-name" title={p.title}>{p.title?.slice(0, 34)}</span>
              <span className="rung-track">
                <span className={`rung-bar ${ratingClass(p.rating)}`} style={{ width: `${Math.max(6, Math.round((p.price / hi) * 100))}%` }} />
      ...
      {picks.length > 0 && (
        <div className="picks">
          <div className="s-label" style={{ marginBottom: 8 }}>LaunchLens picks</div>
