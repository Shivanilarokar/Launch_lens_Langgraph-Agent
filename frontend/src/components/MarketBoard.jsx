...
function Scatter({ products }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="scatter">
      <rect x={PAD.l} y={PAD.t} width={(W - PAD.l - PAD.r) / 2} height={(H - PAD.t - PAD.b) / 2} className="zone" />
    ...
    return (
      <section className="board">
        ...
        <div className="board-empty">
          Ask LaunchLens about a product and this panel fills with live supply analytics:
          price range, a rating scatter, and the picks.
        </div>
      </section>
    )
  }
  ...
  return (
    <section className="board">
      ...
      <div className="board-stats">
        <div className="bstat"><span className="bstat-n">{priced.length}</span><span className="bstat-l">Tracked</span></div>
        <div className="bstat"><span className="bstat-n">{currency} {fmt(lo)}–{fmt(hi)}</span><span className="bstat-l">Price range</span></div>
        <div className="bstat"><span className="bstat-n">★ {avgR}</span><span className="bstat-l">Avg rating</span></div>
      </div>
